...
if (isset($_GET['search'])) {
    $get = $_GET['search'];
    $and = "And (orderscourse_username  LIKE  '%$get%') 
         OR (orderscourse_email      LIKE  '%$get%') 
         OR (orderscourse_id         LIKE  '%$get%')";
} else {
    $and = null;
};
...
if (isset($_GET['searchstatus'])) {
    ...
    if ($get == "wait") {
        $and = "And (orderscourse_status  =  0 )";
    }
    if ($get == "approved") {
        $and = "And (orderscourse_status  =  1 )";
    }
    if ($get == "denied") {
        $and = "And (orderscourse_status  =  10 )";
    }
} else {
    $and = null;
};
            ...
            <div class="panel panel-default panel-custom manage">
                ...
                <div class="panel-body">
                    ...
                    <table class="table  table-bordered table-hover table-responsive table-section">
                        ...
                        ?>
                            <tr>
                                <td class=" "><?php echo $order['courses_name']
                                                ?></td>
                                <td class='<?php
                                            if ($order['orderscourse_status'] == "0") {
                                                echo "wait";
                                            } elseif ($order['orderscourse_status'] == "1") {
                                                echo "approved";
                                            } else {
                                                echo "denied";
                                            }
                                            ?>'><?php echo $order['orderscourse_username']
                                                ?></td>
                                <td class=" hidden-xs"><?php echo $order['orderscourse_phone']
                                                        ?></td>
                                ...
                                <td>
                                    ...
                                    <?php if ($order['orderscourse_status'] == "0") { ?>
                                        ...
                                        <a href="<?= $linkapprovepage ?>?ordersid=<?php echo  $order['orderscourse_id']
                                                                                    ?>" class="btn-success btn-sm mg-h-5 "><i class="fa  fa-check"> </i> <span class="hidden-xs">Approve</span> </a>
                                        <a href="<?= $linkdeniedpage ?>?ordersid=<?php echo  $order['orderscourse_id']
